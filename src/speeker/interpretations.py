...
BUILTIN_INTERPRETATIONS: dict[str, dict] = {
    ...
    # FAILURE is the outcome-named twin of ERROR: callers that classify a turn
    # as a failure (vs. a low-level error) get the same recognizable "something
    # went wrong" descent, so listeners learn one bad-news sound.
    "FAILURE": {
        "type": "notes",
        "notes": [
            {"pitch": "Eb4", "seconds": 0.3},
    ...
    # USER_PROMPT signals "the agent paused to ask *you* something." A rising
    # two-note "ding-dong?" reads as inquisitive -- the upward interval is the
    # auditory shape of a spoken question, distinct from the SUCCESS/FAILURE
    # cues so a waiting prompt is unmistakable across the room.
    "USER_PROMPT": {
        "type": "notes",
        "notes": [
